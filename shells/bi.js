// shells/bi.js — Derive Intelligence Virtual Management Office /* DEKRB_BIJS_V1 */
...
export function initBiSurface(container, me, sub, callbacks) {
  ...
  container.innerHTML = `
    <div class="vmo-shell">
      ...
      <header class="vmo-header">
        <div class="vmo-brand">
          <svg width="27" height="27" viewBox="0 0 100 100" fill="none" style="filter:drop-shadow(0 2px 7px rgba(120,90,255,.4))"><!-- NAV_LOGO_REAL_V1 -->
            <defs><linearGradient id="navtri" x1="22" y1="76" x2="78" y2="24" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2ad4ee"/><stop offset=".52" stop-color="#4f86ff"/><stop offset="1" stop-color="#a855f7"/></linearGradient>
            <filter id="navglow" x="-40%" y="-40%" width="180%" height="180%"><feGaussianBlur stdDeviation="2.4" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
            <g filter="url(#navglow)"><path d="M50 26 L78 74 L22 74 Z" stroke="url(#navtri)" stroke-width="7.5" stroke-linejoin="round" stroke-linecap="round"/><circle cx="78" cy="74" r="6.2" fill="#9333ea"/><circle cx="50" cy="26" r="8.4" fill="#fff"/><circle cx="50" cy="26" r="3.7" fill="#7c3aed"/></g>
          </svg>
        ...
        <nav class="vmo-tabs">
          ...
          ${allowedDepts.includes("finansodasi") ? '<button class="vmo-tab" data-dept="finansodasi" style="--c:#059669" title="Finans Odasi" data-mark="FINANS_TAB_V2"><span>💰</span> Finans</button>' : ''}
          ${allowedDepts.includes("ikodasi") ? '<button class="vmo-tab" data-dept="ikodasi" style="--c:#7c3aed" title="İK Odası" data-mark="IK_TAB_V1"><span>👥</span> İK</button>' : ''}
          ${OFFICERS.map(o => `
            <button class="vmo-tab" data-dept="${o.id}" style="--c:${o.color}">
              <span>${o.emoji}</span> ${o.label}
            </button>`).join("")}

          ${(me.subscriptions || []).some(s => s.moduleId === "saha") ? '<button class="vmo-tab" data-dept="saha" style="--c:#0ea5e9"><span>📍</span> Saha</button>' : ""}
          ${(me.subscriptions || []).some(s => s.moduleId === "saha") ? '<button class="vmo-tab" data-dept="yonportfoy" style="--c:#3fb9a2"><span>🧭</span> Yönetici Portföy</button>' : ""}
        </nav>
    ...
    if ((d.net_risk||[]).length) {
      ...
      (d.net_risk||[]).forEach(function(r){
        ...
        h += '<div style="font-size:12px;color:var(--tx-3);font-family:var(--mono);margin-top:3px">'
           ...
           + (Number(r.krb_borcu) < 0 ? ' · '+(me.tenantAlias||me.tenantName||'firma')+' borcu ' + _M(r.krb_borcu) : '')
  ...
  // ⚠ Tepede GOSTERGE degil TARIH var. Bir gosterge bakilir ve unutulur; bir tarih YAKLASIR.
  {
    ...
    if (_o && !document.getElementById('vmo-room-finans')) {
      ...
      _f.innerHTML = '<iframe id="kokpit-frame" src="/api/bi/kokpit-iki" style="width:100%;height:100%;min-height:82vh;border:0;display:block;background:#05070a" title="Finans Kokpiti"></iframe>'; try{_f.style.padding='0';}catch(e){}
      _o.appendChild(_f);
    }
  }

  // FINANS_TAB_V2 — ayri 'Finans Odasi' sekmesi (dept=finansodasi) -> iframe /api/bi/finans-odasi; eski /api/bi/finans raporuna ve Kokpit'e DOKUNMAZ
  {
    const _o = container.querySelector('.vmo-office');
    if (_o && allowedDepts.includes("finansodasi") && !document.getElementById('vmo-room-finansodasi')) {  /* FINANS_GATE_V1 */
  ...
  container.querySelectorAll(".vmo-tab").forEach(tab => {
    tab.addEventListener("click", () => {
      // SAHA_DESKTOP_V1 — "Saha" sekmesi BI odası değil; masaüstü Saha modülünü açar
      if (tab.dataset.dept === "saha") {
        if (callbacks && typeof callbacks.openSaha === "function") callbacks.openSaha();
        return;
      }
          ...
          function renderCompare() {
            ...
            resultsEl.querySelector('#vmo-sp-ai-btn').addEventListener('click', async function() {
              ...
              try {
                ...
                const prompt = (me.tenantName||'firma')+' lastik firması — kış/yaz maliyet-satış spread analizi. '
        ...
        function renderAll() {
          ...
          resultsEl.querySelector('#vmo-sp-ai-btn').addEventListener('click', async function() {
            ...
            try {
              ...
              const prompt = (me.tenantName||'firma')+' lastik firması — kış lastiği ' + modeLabel + ' / yaz lastiği fark analizi, yıl bazında. '
                ...
                + (me.tenantName||'firma')+' bu trendlere karşı nasıl konumlanmalı? Kısa ve net, Türkçe 4-5 cümle.';
              const resp = await apiFetch('/api/brain/ask', { method: 'POST', body: JSON.stringify({ message: prompt }) });
              aiOut.innerHTML = '<div style="background:var(--bg2,#1e1e1e);border-left:3px solid var(--accent,#3b82f6);padding:12px 16px;border-radius:4px;font-size:13px;line-height:1.6;">'
                + '🤖 ' + (resp && (resp.reply || resp.message) || 'Yanıt alınamadı.') + '</div>';
            } catch(e2) {
        ...
        .then(function(data) {
          ...
          (function() {
            ...
            apiFetch('/api/bi/market/rates').then(function(eco) {
              ...
              var prompt = (me.tenantName||'firma')+' lastik distributorunun Nakit Dongusu verileri (' + cccMonths + ' ay): '
...
[['all','Tüm']].concat((wrap._aracKat||[]).map(function(k){return[k.id,k.label_tr];})).forEach(function(x){var on=faFlt.seg===x[0];h+='<button class="fa-flt" data-fk="seg" data-fv="'+x[0]+'" style="'+(on?BON:BOFF)+'">'+x[1]+'</button>';});
  ...
  wrap.querySelectorAll('.fa-yil').forEach(function(b){
      ...
      if(aiBtn) aiBtn.addEventListener('click',async function(){
        ...
        var prompt=(me.tenantName||'firma')+' Lastik fiyat endeksi — LASSA=100 bazında marka karşılaştırması. Markalar: '+brands+'. Segmentler: '+segs+'. Net maliyet bazlı rekabet pozisyonu hakkında 2-3 kısa cümle yaz. Türkçe.';
        try {
  ...
  function loadRakipRoom(cont, _apiFetch) {
    ...
    room.innerHTML = `
      <div style="flex:1;overflow-y:auto;padding:24px;max-width:1200px;margin:0 auto;width:100%">
        ...
        <div style="display:flex;gap:0;border-bottom:2px solid rgba(255,255,255,0.15);margin-bottom:20px">
          ...
          <button onclick="rfTab('b2b')" id="rf-tab-b2b" style="padding:10px 20px;border:none;background:none;font-size:14px;font-weight:600;cursor:pointer;border-bottom:3px solid transparent;color:#778;margin-bottom:-2px">🏭 B2B Alış${Date.now() < new Date('2026-09-06T00:00:00+03:00').getTime() ? ' <span style="background:#ef4444;color:#fff;font-size:9px;font-weight:800;padding:1px 5px;border-radius:7px;vertical-align:middle;letter-spacing:.03em">YENİ</span>' : ''}</button>
        ...
        <div id="rf-pane-b2b" style="display:none"><div id="rf-b2b-govde"><div style="color:#778;padding:20px">B2B yükleniyor…</div></div></div>
    ...
    window.rfTab = function(tab) {
      ['piyasa','akilli','trend','dot','master','b2b','izleme','alarmlar','ayarlar'].forEach(t => {
      ...
      if (tab === 'b2b')      rfYukleB2B();
      if (tab === 'izleme')   rfYukleIzle();
      if (tab === 'alarmlar') rfYukleAlarm();
      if (tab === 'ayarlar')  { rfYukleStats(); rfYukleAyarlar(); }
    };

    // B2B_TAB_V1 · B2B_LOADER_V62 — desen imzali akilli eslesme + placeholder/tek-ilan temizligi
    ...
    window.rfYukleMaster = async function() {
      ...
      if (ozEl) ozEl.innerHTML = ' &nbsp;·&nbsp; <b>' + (oz.toplam || 0).toLocaleString('tr-TR') + '</b> ürün · '
        + '<b>' + (oz.krb_eslesen || 0).toLocaleString('tr-TR') + '</b> '+(me.tenantAlias||me.tenantName||'firma')+' SKU eşleşti · '
        + '<b>' + (oz.bosluk || 0).toLocaleString('tr-TR') + '</b> boşluk · '
        + '<b>' + (oz.marka || 0) + '</b> marka';

      const aciklama = {
        bosluk: 'Pazarın <b>3+ pazaryerinde</b> sattığı, firmada karşılık gelen SKU bulunamayan ürünler. ⚠ Eşleşme oranı %36 — bu liste <b>incelenecek aday listesidir, sipariş listesi değildir</b>. Bir satır burada çıktı diye stokta yok demek değil; SAP tanımı farklı yazılmış olabilir.',
        fiyat:  (me.tenantAlias||me.tenantName||'firma')+' SKU\'su pazarda bulunan ürünler. <b>Liste fiyatı</b> ile pazarın min/medyan/max\'ı karşılaştırıldı. Liste fiyatı iskonto öncesidir — bu bir marj hesabı değil, <b>müşterinin telefonunda gördüğü fiyat</b>tır.',
      ...
      if (g === 'fiyat') {
        h += '<th style="padding:9px 10px">Marka / Desen</th><th style="padding:9px 10px">Ebat</th>'
           + '<th style="padding:9px 10px">'+(me.tenantAlias||me.tenantName||'firma')+' SKU</th>'
           + '<th style="padding:9px 10px;text-align:right">'+(me.tenantAlias||me.tenantName||'firma')+' liste</th>'
        ...
        h += '<th style="padding:9px 10px">Marka / Desen</th><th style="padding:9px 10px">Ebat</th>'
           ...
           + '<th style="padding:9px 10px;text-align:center">Firmada</th>';
      } else {
        h += '<th style="padding:9px 10px">Marka / Desen</th><th style="padding:9px 10px">Ebat</th>'
  ...
  async function exportOrdersExcel() {
    ...
    try {
      ...
      X.writeFile(wb, `${(me.tenantAlias||me.tenantName||'Firma').replace(/[^A-Za-z0-9]+/g,'_')}_Siparis_${season}_${new Date().toISOString().slice(0,10)}.xlsx`);
    } catch(err) {
...
@media (prefers-reduced-motion: reduce) {
  * { transition: none !important; animation: none !important; }
  ...
  .vmo-brand-text strong { color:#6ea8ff; } /* NAV_LOGO_REAL_V1 wordmark mavi */
