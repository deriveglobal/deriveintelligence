...
import { renderRecommendationsComponent } from '../components/recommendations.js?v=20260609c';
...
import { renderExecutiveReportComponent } from '../components/executive.js?v=20260609d';
import {
  renderHealthPill,
...
function wireConsultantShell(container) {
  ...
  container.addEventListener('click', async (event) => {
    ...
    if (findingsAction) {
      ...
      const findings = safeArray((getSlice('assessment') || {}).findings);
      if (action === 'select-all') {
        consultantState.selectedFindingIds = findings.map((finding) => finding.id).filter(Boolean);
        renderConsultantView('review');
        return;
      }
      if (action === 'unselect-all') {
        consultantState.selectedFindingIds = [];
        renderConsultantView('review');
        return;
      }
      if (action === 'approve-selected') {
        const ids = consultantState.selectedFindingIds || [];
        if (!ids.length) return;
        for (const id of ids) {
          if (callbacks().onApproveFinding) {
            await callbacks().onApproveFinding(id);
          } else {
            await patchConsultantFindingStatus(id, 'Approved');
          }
        }
        consultantState.selectedFindingIds = [];
        renderConsultantView('review');
      ...
      if (action === 'reject-selected') {
        ...
        if (!ids.length) return;
        for (const id of ids) {
          if (callbacks().onRejectFinding) {
            await callbacks().onRejectFinding(id);
          } else {
            await patchConsultantFindingStatus(id, 'Rejected');
          }
        }
        consultantState.selectedFindingIds = [];
        renderConsultantView('review');
    ...
    if (toggleFindingMerge) {
      ...
      consultantState.selectedFindingIds = consultantState.selectedFindingIds || [];
      const idx = consultantState.selectedFindingIds.findIndex((id) => String(id) === String(findingId));
      if (toggleFindingMerge.checked && idx === -1) {
        consultantState.selectedFindingIds.push(findingId);
      } else if (!toggleFindingMerge.checked && idx !== -1) {
        consultantState.selectedFindingIds.splice(idx, 1);
      } else if (idx === -1) {
        consultantState.selectedFindingIds.push(findingId);
      } else {
        consultantState.selectedFindingIds.splice(idx, 1);
    ...
    if (recAction) {
      ...
      if (action === 'approve') callbacks().onApproveRecommendation?.(recId);
      if (action === 'reject') callbacks().onRejectRecommendation?.(recId);
      return;
    }

    const recommendationSelection = event.target.closest('[data-component-select-recommendation]');
    if (recommendationSelection) {
      const recommendationId = recommendationSelection.dataset.componentSelectRecommendation;
      consultantState.selectedRecommendationIds = consultantState.selectedRecommendationIds || [];
      const idx = consultantState.selectedRecommendationIds.findIndex((id) => String(id) === String(recommendationId));
      if (recommendationSelection.checked && idx === -1) {
        consultantState.selectedRecommendationIds.push(recommendationId);
      } else if (!recommendationSelection.checked && idx !== -1) {
        consultantState.selectedRecommendationIds.splice(idx, 1);
      } else if (idx === -1) {
        consultantState.selectedRecommendationIds.push(recommendationId);
      } else {
        consultantState.selectedRecommendationIds.splice(idx, 1);
    ...
    if (recommendationBulkAction) {
      ...
      const recommendations = getSlice('assessment')?.recommendations || [];
      if (action === 'select-all') {
        consultantState.selectedRecommendationIds = recommendations.map((recommendation) => recommendation.id).filter(Boolean);
        renderConsultantView('recommendations');
        return;
      }
      if (action === 'unselect-all') {
        consultantState.selectedRecommendationIds = [];
        renderConsultantView('recommendations');
        return;
      }
      if (action === 'approve-selected') {
        const ids = consultantState.selectedRecommendationIds || [];
        if (!ids.length) return;
        for (const id of ids) {
...
function renderConsultantFindingRows(findings = [], selectedFindingId = '', selectedFindingIds = []) {
  const selectedMergeIds = new Set(safeArray(selectedFindingIds).map((id) => String(id)));
  return `
    <div class="consultant-review-finding-list" role="list">
      ${safeArray(findings).map((finding) => {
        ...
        const checked = selectedMergeIds.has(String(finding.id));
        return `
          <article class="consultant-review-finding-row ${selected ? 'selected' : ''} ${status === 'approved' ? 'approved' : ''} ${status === 'rejected' ? 'rejected' : ''}" role="listitem">
            <input type="checkbox" aria-label="Select finding for merge" data-component-merge-finding="${escapeHtml(finding.id)}" ${checked ? 'checked' : ''} ${selected ? 'disabled' : ''} />
...
function renderConsultantReview() {
  ...
  const selectedCount = new Set(safeArray(consultantState.selectedFindingIds).map((id) => String(id))).size;
  updateViewContainer(`
    ${renderStageLockMessage()}
        ...
        <div class="consultant-review-progress">
          ...
          ${findings.length ? `
            <div class="row-actions compact">
              <button class="text-button" data-findings-action="select-all" type="button">Select All</button>
              <button class="text-button" data-findings-action="unselect-all" type="button">Unselect All</button>
              ${selectedCount > 0 ? `
                <button class="primary-button" data-findings-action="approve-selected" type="button">✓ Approve ${selectedCount} Selected</button>
                <button class="secondary-button" data-findings-action="reject-selected" type="button">× Reject ${selectedCount} Selected</button>
              ` : ''}
            </div>
          ` : ''}
      ...
      <section class="consultant-review-workbench">
        ${findings.length ? `
          <aside class="consultant-review-list-column">
            ${renderConsultantFindingRows(findings, selectedFindingId, consultantState.selectedFindingIds)}
          </aside>
...
function renderConsultantRecommendationRows(recommendations = [], selectedRecommendationId = '', selectedRecommendationIds = []) {
  const checkedIds = new Set(safeArray(selectedRecommendationIds).map((id) => String(id)));
  return `
        ...
        return `
          <article class="consultant-rec-row ${selected ? 'selected' : ''} ${status === 'approved' ? 'approved' : ''} ${status === 'rejected' ? 'rejected' : ''}" role="listitem">
            <label class="review-merge-checkbox recommendation-select-checkbox">
              <input type="checkbox" data-component-select-recommendation="${escapeHtml(recommendation.id)}" ${checkedIds.has(String(recommendation.id)) ? 'checked' : ''}>
              <span>Select</span>
            </label>
...
function renderConsultantRecommendations() {
  ...
  const selectedRecommendationCount = new Set(safeArray(consultantState.selectedRecommendationIds).map((id) => String(id))).size;
  updateViewContainer(`
    ${renderStageLockMessage()}
    <section class="consultant-stage consultant-recommendations-stage">
      <div class="owner-section-head">
        <div><span class="eyebrow">Recommendations</span><h2>Action quality gate</h2><p>Approve the recommendations that should reach delivery, then confirm the roadmap and progress memory.</p></div>
        ${recommendations.length ? `
          <div class="row-actions compact">
            <button class="text-button" data-recommendations-action="select-all" type="button">Select All</button>
            <button class="text-button" data-recommendations-action="unselect-all" type="button">Unselect All</button>
            ${selectedRecommendationCount > 0 ? `
              <button class="primary-button" data-recommendations-action="approve-selected" type="button">✓ Approve ${selectedRecommendationCount} Selected</button>
              <button class="secondary-button" data-recommendations-action="reject-selected" type="button">× Reject ${selectedRecommendationCount} Selected</button>
            ` : ''}
          </div>
        ` : ''}
      </div>
      <section class="consultant-rec-workbench">
        ${recommendations.length ? `
          <aside class="consultant-rec-list-column">
            ${renderConsultantRecommendationRows(recommendations, selectedRecommendationId, consultantState.selectedRecommendationIds)}
          </aside>
