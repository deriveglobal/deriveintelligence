...
export function initTenantAdminSurface(container, me, callbacks) {
  ...
  const MODULES = {
    saha: {
      ...
      segment: true,
      groups: [
        ["Sekmeler", [["ozet", "📊 Özet"], ["ciro", "💰 Ciro"], ["risk", "🚨 Risk"], ["etki", "📈 Saha ROI"], ["portfoy", "🩺 Portföy"], ["rotam", "🌅 Rotam"], ["kapsam", "📍 Kapsam"], ["ziyaretanaliz", "🔬 Ziyaret Analizi"], ["pipeline", "💼 Pipeline"], ["pazar", "🎯 Pazar"]]],  /* SEKME_CAP_V1 — saha sekmeleri = birincil yetki */
        ...
        ["Zeka & Pazar", [["harita", "Harita"], ["piyasa", "Piyasa"], ["rakip", "Rakip"], ["oneriler", "Öneriler"], ["sahasesi-tum", "Saha Sesi · Tüm KRB"], ["sahasesi-temsilci", "Saha Sesi · Temsilci"]]],  /* SAHASESI_CAP_V1 */
        ["Yönetim & Analiz", [["rapor", "Rapor"], ["ekip-plan", "Ekip Planı"], ["kokpit", "Kokpit"], ["ceo", "CEO"], ["rep-aktivite", "Aktivite"], ["memnuniyet", "Memnuniyet"], ["yon-portfoy", "Yönetici Portföy"]]] /* EKIP_PLAN_TA_V1 */  /* YONPORTFOY_MATRIS_V1 */  /* SEKME_CAP_V1 relokasyon: ciro/risk/rotam/kapsam -> Sekmeler */  /* MEMNUNIYET_IZIN_V1 */,  /* KAPSAM_TAD_V1 */  /* IZIN_YENIMODUL_V1 */
        ["İletişim", [["duyurular", "Duyurular"], ["mesajlar", "Mesajlar"]]],
        ["Sistem & Erişim", [["temsilciler", "Ekip"], ["sistem", "Sistem"]]]
      ],
      lock: ["ozet", "temsilciler", "sistem"],  /* SEKME_CAP_V1 — ozet kilitli */
      gmRole: "admin"
    },
    intelligence: {
      label: "Management",
      ...
      groups: [
        ...
        ["Odalar", [["finansodasi", "Finans Odası"], ["ikodasi", "İK Odası"]]]  /* FINANS_IZIN_V1 */
      ],
      lock: [],
      gmRole: "admin"
    }
  ...
  const DEFAULTS = {
    saha: {
      rep: ["bugun", "ziyaretler", "plan", "musteriler", "musterikart", "teklif", "notlarim", "rep-brain", "piyasa", "rakip", "rapor", "duyurular", "mesajlar", "oneriler", "rotam", "etki", "portfoy", "pipeline", "pazar", "ziyaretanaliz", "ozet"],  /* SEKME_CAP_V1 */  /* YETKI_FAZ1 */  /* IZIN_YENIMODUL_V1 */  /* ZA_YETKI_V1 */
      manager: ["bugun", "ziyaretler", "plan", "musteriler", "musterikart", "teklif", "notlarim", "rep-brain", "piyasa", "rakip", "rapor", "duyurular", "mesajlar", "oneriler", "harita", "kokpit", "ceo", "ciro", "risk", "rotam", "kapsam", "etki", "portfoy", "pipeline", "pazar", "ziyaretanaliz", "sahasesi-tum", "sahasesi-temsilci", "ozet", "ekip-plan", "yon-portfoy"],  /* EKIP_PLAN_TA_V1 */  /* YONPORTFOY_MATRIS_V1 */  /* SEKME_CAP_V1 */  /* YETKI_FAZ1 */  /* KAPSAM_TAD_V1 */  /* IZIN_YENIMODUL_V1 */  /* SAHASESI_CAP_V1 */
      admin: modTools("saha")
    },
    intelligence: {
      viewer: [],
  ...
  container.innerHTML = `
    ...
    <aside class="ta-sidebar">
      ...
      <nav class="ta-nav">
        ...
        <!-- YETKI_FAZ5B: eski "İzinler" grid nav'dan kaldırıldı (Bölümler/Kişiler/Roller/Denetim yerini aldı; renderPermissions kod olarak durur) -->
        ${navBtn("bolumler", "Bölümler", '<path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/>')}  <!-- YETKI_FAZ2B -->
        ...
        ${subModules().includes("saha") ? `<div class="ta-nav-label">Müşteriler</div>
        ...
        ${navBtn("yetki-log", "Yetki Günlüğü", '<path d="M12 8v4l3 3"/><circle cx="12" cy="12" r="9"/>')}  <!-- YETKI_FAZ5B -->
        ...
        ${navBtn("veri", "Veri", '<path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>')}
  ...
  async function renderView(view) {
    ...
    const map = {
      ...
      /* YETKI_FAZ5B: "permissions" (eski İzinler grid) nav'dan kaldırıldı; renderPermissions kod olarak durur */
      bolumler: ["Bölümler", "Org bölümleri — yetki şablonu + toplu uygula", renderBolumler],  /* YETKI_FAZ2B */
      ...
      "yetki-log": ["Yetki Günlüğü", "Yetki değişiklik günlüğü — kim, kime, ne zaman, ne", renderYetkiLog],  /* YETKI_FAZ5B */
      sistem: ["Sistem", "Saha hata & sistem raporu", renderSistem],
      memnuniyet: ["Memnuniyet", "Saha memnuniyet nabzi", renderMemnuniyet],
      veri: ["Veri", "ERP dosya yükleme — kalite kapıları + sağlık", renderVeri],  /* VERI_YONETIM_V1 */
      yardim: ["Yetki Rehberi", "Yetki modülü — teknik dokümantasyon", renderYardim]  /* YETKI_YARDIM_V1 */
    };
  ...
  // ─── Overview ──────────────────────────────────────────────────────────────
  async function renderVeri(content) {  /* VERI_YONETIM_V1 */
  ...
  // ─── Müşteri Atama (MUSTERI_ATAMA_V1 · fix MUSTERI_ATAMA_FIX_V1) ─────────────
  ...
  // ── YETKI_FAZ2B — Bölümler (org bölüm/üyelik + şablon + toplu uygula) ──
  async function renderBolumler(content) {
    const { departments } = await apiFetch("/api/tenant/departments");
    const groups = [...((MODULES.saha && MODULES.saha.groups) || []), ["Odalar", [["ikodasi","İK Odası"],["finansodasi","Finans Odası"]]]];  /* IK_BOLUM_V1 */
    ...
    async function drawMembers(d) {
      ...
      box.innerHTML=`<div class="tb-members"><div class="tb-lbl">👥 Üyeler — işaretle → aşağıda "Kaydet & uygula" da kaydeder</div>  <!-- UX_YETKI_V1 --><div class="tb-mgrid">${users.map(u=>`<label class="tb-mrow"><input type="checkbox" data-uid="${u.id}"${u.uye?' checked':''}><span>${esc(u.ad)}</span></label>`).join("")}</div><div class="tb-foot2"><button class="ta-btn ta-btn-xs" id="tb-msave">Üyeliği kaydet</button><span class="tb-msg" id="tb-msg2"></span></div></div>`;
  ...
  // ── YETKI_FAZ2CD — Kişiler editörü + Roller referansı ──
  async function renderKisiler(content) {
    ...
    function drawMain() {
      ...
      const grpHtml = cfg.groups.map(g=>{ const items=g[1]; const on=items.filter(t=>has.has(t[0])||guard).length;
        return `<div class="tk-grp"><div class="tk-grp-h"><span class="tk-gn">${esc(g[0])}</span><span class="tk-gc">${on}/${items.length}</span><label class="tk-master"><input type="checkbox" data-gmaster="${esc(g[0])}"${on===items.length?' checked':''}${guard?' disabled':''}> tümü</label></div><div class="tk-grid">${items.map(t=>{ const lock=guard&&cfg.lock.includes(t[0]); const chk=has.has(t[0])||guard||lock; const tag=(chk&&!guard)?((Array.isArray(perms.personal_grant)&&perms.personal_grant.includes(t[0]))?'<span class="tk-tag ovr">kişisel</span>':'<span class="tk-tag def">bölüm</span>'):'';  /* UX_YETKI_V1 */ return `<label class="tk-tog${chk?' on':''}" data-grp="${esc(g[0])}"><input type="checkbox" class="tk-cap" data-cap="${t[0]}"${chk?' checked':''}${(guard||lock)?' disabled':''}><span>${esc(t[1])}${tag}</span></label>`; }).join("")}</div></div>`; }).join("");
  ...
  async function renderYetkiDenetim(content) {  /* YETKI_FAZ5 */
  ...
  async function renderYetkiLog(content) {  /* YETKI_FAZ5B */
    content.innerHTML = `<div class="ta-loading"><div class="ta-spin"></div>Yükleniyor…</div>`;
  ...
  async function renderInvite(content) {
    ...
    content.innerHTML = `<div class="ta-two">
      <div class="ta-card ta-form">
        ...
        <form id="ta-invf">
          ...
          <label class="ta-lbl">Profil (hızlı)</label>
          <select id="inv-prof" class="ta-input"><option value="">— Profil seç (ya da aşağıdan elle) —</option><option value="yonetim">Yönetim — tüm BI (admin)</option><option value="yonetim_full">Yönetim (BI + Saha) — çift modül</option><option value="bi_izleyici">BI İzleyici — salt-okunur</option><option value="saha_temsilci">Saha Temsilcisi (rep)</option><option value="saha_mudur">Saha Müdürü (manager)</option></select>
    ...
    let _multiSel = null; /* MULTI_MODULE_INVITE_CLIENT_V1 */
    const _allDeps = (mod) => (MODULES[mod] ? MODULES[mod].groups.flatMap(g => g[1]).map(t => t[0]) : []);
    const profSel = content.querySelector("#inv-prof");
    if (profSel) profSel.addEventListener("change", () => { /* INVITE_PROFIL_V1 */
      _multiSel = null; modSel.disabled = false;
      const MULTI = { yonetim_full: [{ module_id: "intelligence", module_role: "admin" }, { module_id: "saha", module_role: "manager" }] };
      if (MULTI[profSel.value]) {
        _multiSel = MULTI[profSel.value].map(m => ({ module_id: m.module_id, module_role: m.module_role, permissions_json: { departments: _allDeps(m.module_id) } }));
        modSel.value = ""; syncMod(); modSel.disabled = true;
        subsBox.innerHTML = `<div class="ta-subhead" style="margin-top:18px"><label class="ta-lbl">Bu profil şu modülleri verir</label></div><div class="ta-subgrid">` + _multiSel.map(m => `<span class="ta-tag">${esc(moduleLabel(m.module_id))} · ${esc(m.module_role)} · tüm bölümler</span>`).join("") + `</div>`;
        return;
      }
      const P = { yonetim:{mod:"intelligence",role:"admin"}, bi_izleyici:{mod:"intelligence",role:"viewer"}, saha_temsilci:{mod:"saha",role:"rep"}, saha_mudur:{mod:"saha",role:"manager"} }[profSel.value];
      if (!P) return;
      modSel.value = P.mod; syncMod(); roleSel.value = P.role;
      subsBox.querySelectorAll(".inv-sub").forEach(c => { c.checked = true; }); updSubCount();
    });
    modSel.addEventListener("change", () => { _multiSel = null; if (profSel) profSel.value = ""; });
    ...
    content.querySelector("#ta-invf").addEventListener("submit", async e => {
      ...
      try {
        const _payload = _multiSel ? { email, modules: _multiSel } : { email, module_id: modId || undefined, module_role: role || undefined, permissions_json: subs.length ? { departments: subs } : {} };
        const data = await apiFetch("/api/tenant/users/invite", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(_payload) });
        ...
        content.querySelector("#ta-invf").reset(); _multiSel = null; modSel.disabled = false; if (profSel) profSel.value = ""; syncMod();
  ...
  // Denetim: Memnuniyet (saha nabiz) — MEMNUNIYET_PANEL_V1
  function renderYardim(content) {  /* YETKI_YARDIM_V1 */
...
function taStyles() {
  return `
    ...
    .ta-input{width:100%;padding:11px 13px;border:1px solid #d7dbe3;border-radius:10px;font-size:14px;outline:none;background:#fff;color:#12182a;color-scheme:light;transition:.14s} /* INVITE_INPUT_FIX_V1 */
    .ta-input option{color:#12182a;background:#fff}
    .ta-input:focus{border-color:#12182a;box-shadow:0 0 0 3px rgba(18,24,42,.08)}
    .ta-subhead{display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap} /* INVITE_SELALL_V1 */
