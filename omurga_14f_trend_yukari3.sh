#!/usr/bin/env bash
# OMURGA 14f — trendi net sermaye altına: ÖDEME TAKVİMİ etiketinden GERİYE en yakın if(OD.length). bi.js.
set -uo pipefail
cd /opt/krb-assessment || exit 1
hr(){ printf '\n════════ %s ════════\n' "$1"; }

hr "1. YEDEK"
cp shells/bi.js shells/bi.js.bak_trend6
echo "  ✅ .bak_trend6"

hr "2. TAŞI (Python — ÖDEME TAKVİMİ etiketinden geriye en yakın if)"
python3 - <<'PY'
import sys
...
if u.count(MARK)!=1: print("  ❌ ödeme takvimi etiketi:",u.count(MARK)); sys.exit(2)
# 1) dipteki container'ı kaldır
u = u.replace(CONTAINER, "", 1)
# 2) ÖDEME TAKVİMİ etiketinin konumu → ondan GERİYE en yakın if(OD.length) = ödeme takvimi if'i
mp = u.index(MARK)
ifp = u.rindex("if (OD.length)", 0, mp)
u = u[:ifp] + CONTAINER + "\n      " + u[ifp:]
open(p,'w',encoding='utf-8').write(u)
print("  ✅ taşındı (doğru if — ödeme takvimi öncesi)")
PY
PYRC=$?
if [ $PYRC -ne 0 ]; then echo "  ⚠ başarısız, geri al"; cp shells/bi.js.bak_trend6 shells/bi.js; exit 1; fi

hr "3. SÖZDİZİMİ + KONUM"
cp shells/bi.js /tmp/_bic.mjs
node --check /tmp/_bic.mjs && echo "  ✅ sözdizimi OK" || { echo "  ❌ bozuk, geri al"; cp shells/bi.js.bak_trend6 shells/bi.js; exit 1; }
...
if [ -n "$CONT" ] && [ "$CONT" -gt "$NET" ] && [ "$CONT" -lt "$ODE" ]; then
  ...
  echo "  ❌ konum yanlış — geri al"; cp shells/bi.js.bak_trend6 shells/bi.js; exit 1
fi

hr "4. AYAK İZİ + DEPLOY"
docker exec -i krb-assessment-postgres psql -U assessment_app -d assessment_platform -c \
 "INSERT INTO bi_insa_gunlugu (adim,ne,neden,detay) VALUES ('omurga_14f','Marka Trendi net sermaye altina (dogru if)','Mansset gorunur','{}');" >/dev/null 2>&1 || true
echo "  ✅ ayak izi"
echo "  ▶ DEPLOY: cd /opt/krb-assessment && docker build -t krb-assessment:secure . && docker compose up -d --force-recreate krb-assessment"

hr "BITTI — trend doğru konumda. Deploy yukarıda."
