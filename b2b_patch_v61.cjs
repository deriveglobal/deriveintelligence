#!/usr/bin/env node
// bi.js B2B loader'i V6.1 (desen imzasi + 4'lu set temizligi) yapar. .cjs → require calisir.
...
if(src.indexOf('B2B_LOADER_V61')>=0){console.log('⏭  Zaten V6.1 kurulu — atlandi');process.exit(0);}
...
const marker='    // B2B_TAB_V1 · B2B_LOADER_V61 — desen imzali akilli eslesme + set temizligi';
const out=lines.slice(0,si).concat([marker], loader.split('\n'), [''], lines.slice(ei)).join('\n');
const bak=BI+'.bak_v61_'+Date.now();
...
console.log('✓ V6.1 yazildi · yedek: '+bak+' · '+lines.length+'→'+out.split('\n').length+' satir');
