-- DUZELTME: rejim testi model-bagimsiz hale getirildi.
-- v1 esigi bant_genislik/2 idi; bu, genis bantli kotu modeli kirilmadan muaf
-- tutuyordu (TAMIR adet: mevsimsel_trend 0,77 gecti, birlesik_medyan 0,53 kaldi).
-- v2: iki yarinin ceyrekler-arasi araliklari ayriksa kirilma vardir. Sabit yok,
-- model yok; sadece serinin kendi dagilimi.
...
a AS (
  SELECT u.tid, u.varlik_tipi, u.varlik_kodu, u.nesne,
    percentile_cont(0.5)  WITHIN GROUP (ORDER BY u.v) FILTER (WHERE NOT u.son_yari) AS i_med,
    percentile_cont(0.5)  WITHIN GROUP (ORDER BY u.v) FILTER (WHERE     u.son_yari) AS s_med,
    percentile_cont(0.25) WITHIN GROUP (ORDER BY u.v) FILTER (WHERE NOT u.son_yari) AS i_q1,
    percentile_cont(0.75) WITHIN GROUP (ORDER BY u.v) FILTER (WHERE NOT u.son_yari) AS i_q3,
    percentile_cont(0.25) WITHIN GROUP (ORDER BY u.v) FILTER (WHERE     u.son_yari) AS s_q1,
    percentile_cont(0.75) WITHIN GROUP (ORDER BY u.v) FILTER (WHERE     u.son_yari) AS s_q3,
    count(*) FILTER (WHERE NOT u.son_yari) AS n_i,
    count(*) FILTER (WHERE     u.son_yari) AS n_s
   FROM u WHERE u.v IS NOT NULL GROUP BY 1,2,3,4)
SELECT a.tid AS tenant_id, a.varlik_tipi, a.varlik_kodu, a.nesne,
       a.i_med AS ilk_yari, a.s_med AS son_yari, a.n_i AS n_ilk, a.n_s AS n_son,
       CASE WHEN a.i_med > 0 THEN abs(a.s_med - a.i_med)/a.i_med END AS kirilma_orani,
       a.i_q1 AS ilk_q1, a.i_q3 AS ilk_q3, a.s_q1 AS son_q1, a.s_q3 AS son_q3,
       (a.i_q1 > a.s_q3 OR a.s_q1 > a.i_q3) AS kirildi
  FROM a;
 ...
 WITH ham AS (
   SELECT i.tenant_id, i.nesne, i.varlik_tipi, i.varlik_kodu, i.sinif, i.ufuk_gun,
       ...
       AND COALESCE(i.bant_genislik,99::numeric) <= bi_parametre_coz('kapi_max_bant_genislik'))
          AS istatistik_gecti,
          -- MODEL-BAGIMSIZ: ayni hucredeki her model icin ayni deger.
