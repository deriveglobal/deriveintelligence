#!/bin/sh
# CRON_MULTITENANT_SINYAL_V1 — sinyal_motor_v1 TUM satis'li kiracilar icin.
...
for T in $TENANTS; do
  [ -z "$T" ] && continue
  sed '/^\\set t /d' /opt/krb-assessment/sinyal_motor_v1.sql | $RUN -v t="$T" -v ON_ERROR_STOP=0 >/dev/null 2>&1
  echo "  sinyal_v1: $T"
done
echo "[$(date '+%F %T')] sinyal_motor_v1 tamam (tum kiracilar)"
