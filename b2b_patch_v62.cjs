#!/usr/bin/env node
// bi.js B2B loader'i V6.2 (placeholder + tek-ilan temizligi) yapar. .cjs → require calisir.
const fs=require('fs'), cp=require('child_process');
const BI='/opt/krb-assessment/shells/bi.js';
const LOADER='/opt/krb-assessment/b2b_loader.js';
const START='// B2B_TAB_V1 · B2B_LOADER';
const END ='// ═══ URUN MASTER (URUN_MASTER_V1)';
if(!fs.existsSync(LOADER)){console.error('HATA: '+LOADER+' yok — once scp et');process.exit(1);}
let src=fs.readFileSync(BI,'utf8');
if(src.indexOf('B2B_LOADER_V62')>=0){console.log('⏭  Zaten V6.2 kurulu — atlandi');process.exit(0);}
...
if(si<0||ei<0||ei<=si){console.error('HATA: marker sirasi bozuk');process.exit(1);}
...
const marker='    // B2B_TAB_V1 · B2B_LOADER_V62 — desen imzali akilli eslesme + placeholder/tek-ilan temizligi';
const out=lines.slice(0,si).concat([marker], loader.split('\n'), [''], lines.slice(ei)).join('\n');
const bak=BI+'.bak_v62_'+Date.now();
...
if(origOk){
  ...
  try{cp.execSync('node --check '+LOADER,{stdio:'pipe'});console.log('✓ loader fragmani OK');}
  catch(e){fs.writeFileSync(BI,src);console.error('✗ loader SYNTAX FAIL — geri alindi.');process.exit(1);}
}
console.log('✓ V6.2 yazildi · yedek: '+bak+' · '+lines.length+'→'+out.split('\n').length+' satir');
console.log('Simdi: docker build -t krb-assessment:secure . && docker compose up -d --force-recreate krb-assessment');
