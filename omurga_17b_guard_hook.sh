#!/usr/bin/env bash
# OMURGA 17b (#127) — guard hook, girinti-bağımsız anchor. server.
set -uo pipefail
cd /opt/krb-assessment || exit 1
hr(){ printf '\n════════ %s ════════\n' "$1"; }

hr "1. YEDEK"
cp server_container.mjs server_container.mjs.bak_hook2
echo "  ✅ .bak_hook2"

hr "2. YAMA (girinti içermeyen substring anchor)"
python3 - <<'PY'
import sys
...
if s.count(OLD)!=1: print("  ❌ anchor:",s.count(OLD)); sys.exit(2)
NEW=(
...
PY
PYRC=$?
if [ $PYRC -ne 0 ]; then echo "  ⚠ başarısız, geri al"; cp server_container.mjs.bak_hook2 server_container.mjs; exit 1; fi

hr "3. SÖZDİZİMİ + DEĞİŞİKLİK"
node --check server_container.mjs && echo "  ✅ sözdizimi OK" || { echo "  ❌ bozuk, geri al"; cp server_container.mjs.bak_hook2 server_container.mjs; exit 1; }
echo "  guard çağrısı: $(grep -c 'veri_saglik_kapisi(\$1::uuid) AS alarm' server_container.mjs)"
echo "  yanıtta saglik: $(grep -c 'tazelenen, saglik }' server_container.mjs)"

hr "4. AYAK İZİ + DEPLOY"
docker exec -i krb-assessment-postgres psql -U assessment_app -d assessment_platform -c \
 "INSERT INTO bi_insa_gunlugu (adim,ne,neden,detay) VALUES ('omurga_17_#127hook','guard yukle endpointine baglandi (otomatik)','Her ERP yuklemesinde denetim + alarm yanitta','{}');" >/dev/null 2>&1 || true
echo "  ✅ ayak izi"
echo "  ▶ DEPLOY: cd /opt/krb-assessment && docker build -t krb-assessment:secure . && docker compose up -d --force-recreate krb-assessment"

hr "BITTI — guard her yüklemeye otomatik bağlı. Deploy + test dosyası ile alarm görülür."
