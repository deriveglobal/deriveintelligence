#!/usr/bin/env bash
# OMURGA 49d — v5.1: döviz kısa-seri uyarısı kullanıldıysa guven'i 'kismi'ye indir (dürüst etiket).
# Tam fonksiyon yeniden yazımı; sadece caveat_used + guven mantığı değişti.
set -uo pipefail
PSQL="docker exec -i krb-assessment-postgres psql -U assessment_app -d assessment_platform"
T='f8a5d20f-ecf8-4ce2-a492-69268fbb03fa'
hr(){ printf '\n════════ %s ════════\n' "$1"; }

hr "1. sebep_arastir_marj v5.1 (guven dürüstlük düzeltmesi)"
...
DECLARE
  ...
  sebep text; aksiyon text; guv text; soru_id uuid; kok_belirsiz boolean := false; caveat_used boolean := false;
BEGIN
  WITH a AS (
    SELECT kalem_kodu,
  ...
  ELSE
    ...
    IF kok_belirsiz THEN
      ...
      guv:='kismi';
    ELSIF caveat_used THEN guv:='kismi';   -- döviz kısa-seri hipotezi → dürüst etiket
...
hr "2. guven artık dürüst: CONTINENTAL/LASSA/HERKUL → 'kismi' (döviz hipotez)"
$PSQL -c "SELECT m marka, (sebep_arastir_marj('$T'::uuid,m))->>'guven' guven FROM (VALUES ('CONTINENTAL'),('LASSA'),('HERKUL')) v(m);" 2>&1 | sed 's/^/  /'

hr "BITTI — v5.1: etiket ile kanıt uyumlu (kısa-seri hipotez = kısmi)."
