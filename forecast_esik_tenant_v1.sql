-- Kapi esikleri artik kodda degil, her tenant x nesne icin satirda.
-- Deger degismiyor (davranis-notr); degisen sey: satir yoksa kapi ACILMIYOR, KAPANIYOR.
INSERT INTO bi_tahmin_yayin_esik
  (tenant_id, nesne, min_gozlem, min_donem, max_medyan_mape, min_bant_kapsama, max_bant_kapsama)
SELECT t.tenant_id, n.nesne, 12, 12, 25, 70, 95
  FROM bi_tenant_listesi() t CROSS JOIN (VALUES ('adet'),('ciro')) n(nesne)
 WHERE NOT EXISTS (SELECT 1 FROM bi_tahmin_yayin_esik e
                    WHERE e.tenant_id=t.tenant_id AND e.nesne=n.nesne);

-- Once dogrula, sonra COALESCE'leri kaldir: eksik satir varsa hic dokunma
DO $g$
DECLARE eksik integer;
BEGIN
  SELECT count(*) INTO eksik
    FROM bi_tenant_listesi() t CROSS JOIN (VALUES ('adet'),('ciro')) n(nesne)
    LEFT JOIN bi_tahmin_yayin_esik e ON e.tenant_id=t.tenant_id AND e.nesne=n.nesne
...
CREATE OR REPLACE VIEW v_tahmin_yayin_kapisi AS
 WITH ham AS (
   SELECT i.tenant_id, i.nesne, i.varlik_tipi, i.varlik_kodu, i.sinif, i.ufuk_gun,
          ...
          -- COALESCE YOK: esik satiri yoksa sonuc NULL, uretici NULL'i yayin saymaz
          (i.n >= e.min_gozlem
