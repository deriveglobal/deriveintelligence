...
async function _ekLoad() {
  ...
  try {
    ...
    const row = (l, v) => '<div style="display:flex;justify-content:space-between;gap:10px;align-items:baseline;padding:7px 0;border-bottom:1px solid #f1f5f9;font-size:13px"><span style="color:#64748b;flex:0 1 auto;min-width:0">' + l + '</span><b style="flex:1 1 auto;min-width:0;text-align:right;word-break:break-word">' + v + '</b></div>'; /* EBATKART_TASMA_V2 */
    let h = "";
    ...
    if (d.alicilar && d.alicilar.length) {
      ...
      h += d.alicilar.map(a => '<div style="display:flex;justify-content:space-between;gap:10px;align-items:baseline;padding:6px 0;border-bottom:1px solid #f8fafc;font-size:12px"><span style="flex:1;min-width:0;overflow-wrap:anywhere">' + esc(a.musteri || "—") + '</span><b style="flex-shrink:0;text-align:right;white-space:nowrap">' + money(a.fiyat) + ' <span style="color:#94a3b8;font-weight:400">' + (a.vade != null ? "· " + a.vade + " gun" : (a.odeme ? "· " + esc(a.odeme) : "")) + '</span></b></div>').join("");
    }
...
async function vKokpitMobil() { /* FAZD_OWNERLENS */
  const m = main();
  ...
  try {
    ...
    (async () => {
      try {
        ...
        tabBadge("oneriler", oneriler.filter(o => o.okunmamis && ["YENI", "INCELENIYOR"].includes(o.durum)).length); /* ONERI_BADGE_FIX */
      } catch {}
    })();
  ...
  (async () => {
    ...
    const render = (d) => {
      ...
      if (d.son_alimlar && d.son_alimlar.length) {
        h += `<div style="font-size:11px;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:.3px;margin:10px 0 4px">Son ${d.son_alimlar.length} Alım${["manager","admin"].includes(S.role)?' <span style="color:#94a3b8;font-weight:400;text-transform:none">· ödediği → önerilen</span>':''}</div>` /* SMARTFIYAT_UI_V2 */ + d.son_alimlar.map(a => `<div style="display:flex;align-items:center;gap:10px;padding:7px 2px;border-bottom:1px solid #f1f5f9"><div style="flex:1;min-width:0"><div style="color:#0f172a;font-weight:600;font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc([a.marka, a.ebat].filter(Boolean).join(" ")) || "—"}</div><div style="color:#94a3b8;font-size:10px">${dt(a.fatura_tarihi)}${a.miktar != null ? " · " + Number(a.miktar).toLocaleString("tr-TR") + " ad" : ""}</div></div><div style="flex:0 0 auto;text-align:right"><div style="color:#475569;font-size:13px;font-weight:600">${a.birim_fiyat != null ? Number(a.birim_fiyat).toLocaleString("tr-TR", { maximumFractionDigits: 0 }) + " ₺" : "—"}</div>${["manager","admin"].includes(S.role)?`<div data-fo-oneri="${esc(a.kalem_kodu||'')}" style="font-size:11px;color:#94a3b8;font-weight:700">·</div>`:""}</div></div>`).join("");
      }
    ...
    el.innerHTML = `
      <div style="padding:10px 12px 80px">
        ...
        <div style="background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:14px;margin-bottom:16px">
          ...
          <select id="ss-choro-sec" style="font-size:12px;padding:6px 8px;width:100%;box-sizing:border-box;margin-bottom:8px;border:1px solid #e5e7eb;border-radius:8px;background:#fff;color:#0f172a"><option value="">🎨 Renklendir: kapalı</option><option value="ciro">🎨 Ciro (dönem)</option><option value="adet">🎨 Adet (dönem)</option><option value="ciro_tuketici">🎨 Tüketici ciro (dönem)</option><option value="ciro_ticari">🎨 Ticari ciro (dönem)</option><option value="net_gecikmis">🎨 Net gecikmiş alacak (güncel)</option><option value="net_risk">🎨 Net risk (güncel)</option><option value="cari_bakiye">🎨 Cari bakiye (güncel)</option></select>
          ...
          <div id="ss-harita" style="position:relative;background:#eef2f7;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;height:440px">
...
(function () {
  try {
    ...
    function sc() { if (document.querySelector(".ceo-wrap")) return null; /* VOICE_FIX_2 */ return document.getElementById("saha-main"); }
    ...
    function refresh() {
      ...
      if (nv && getComputedStyle(nv).display !== "none") { var t = nv.querySelector(".saha-tab.on"); if (t) { t.click(); return; } }
    }
