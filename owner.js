...
function isCriticalOwnerFinding(finding = {}) {
  return String(finding.finding_strength || finding.findingStrength || '').toLowerCase() === 'high'
    || ownerSeverityValue(finding) >= 4;
}
...
function ownerRecommendationPhase(recommendation = {}) {
  return recommendation.roadmap_phase || recommendation.phase || recommendation.roadmap_relevance || recommendation.priority || 'Priority action';
}

function ownerRecommendationOwner(recommendation = {}) {
  return recommendation.owner || recommendation.accountable_owner || recommendation.owner_role || recommendation.responsible_role || 'Owner TBD';
}
...
function renderOwnerOverview() {
  ...
  const health = ownerOperationalHealth(assessment, executive);
  ...
  ownerViewContainer.innerHTML = `
    <section class="owner-executive-brief">
      ...
      <section class="owner-signal-metrics" aria-label="Signal metrics">
        ...
        <article><strong>${health === null || health === undefined || health === '' ? '--' : escapeHtml(String(health))}</strong><span>Operational health score</span></article>
      </section>
