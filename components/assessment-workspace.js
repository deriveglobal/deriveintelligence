...
} from './recommendations.js?v=20260609c';
import {
...
function renderReviewFindingList(findings = [], selectedFindingId, callbacks = {}) {
  const selectedFindingIds = new Set(normalizeArray(callbacks.selectedFindingIds).map((id) => String(id)));
  return `
    <div class="review-finding-list" role="list">
      ${normalizeArray(findings).map((finding) => {
        ...
        const checked = selectedFindingIds.has(String(finding.id));
        return `
    ...
    <article class="review-finding-detail">
      <header>
        <div>
          ...
          <p>${escapeHtml(finding.business_domain || finding.domain || 'Domain')} · ${escapeHtml(finding.assessment_category || finding.category || 'Category')} · ${renderGlossaryTooltip('finding_severity', 'Severity', '1=minor, 5=critical business impact.')} ${escapeHtml(finding.severity ?? '-')}</p>
        </div>
      ...
      <div class="review-score-grid">
        <article><span>${renderGlossaryTooltip('confidence_score', 'Confidence Score', 'How confident the AI is that this finding is real.')}</span><strong>${escapeHtml(finding.confidence ?? '-')}/5</strong></article>
        <article><span>${renderGlossaryTooltip('role_divergence', 'Role Divergence', 'How much roles disagree. Higher = stronger misalignment.')}</span><strong>${escapeHtml(finding.overall_stakeholder_agreement_score || finding.stakeholder_agreement_score || 1)}/5</strong></article>
      </div>
      ...
      <div class="review-decision-actions">
        ...
        `}
        ${callbacks.onDeleteFinding ? `<button class="text-button danger" data-component-finding-action="delete" data-finding-id="${escapeHtml(finding.id)}" type="button">Delete finding</button>` : ''}
      </div>
    </article>
  `;
}
...
function renderWorkspaceReviewStage(
  ...
  const selectedCount = new Set(normalizeArray(callbacks.selectedFindingIds).map((id) => String(id))).size;
...
function renderWorkspaceRecommendationsStage(
  ...
  const allRecommendations = normalizeArray(recommendations);
  const selectedRecommendationCount = new Set(normalizeArray(callbacks.selectedRecommendationIds).map((id) => String(id))).size;
  return `
    <section class="assessment-workspace-stage assessment-workspace-recommendations">
      <section class="panel">
        <div class="panel-header">
          ...
          <div class="row-actions compact">
            <span>${allRecommendations.length} generated</span>
        ...
        ${renderRecommendationsComponent(
          ...
          emptyStateHtml,
          {
            ...
            selectedRecommendationIds: callbacks.selectedRecommendationIds
          }
        )}
      </section>
      <section class="panel">
