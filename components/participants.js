...
export function renderEditParticipantModal(participant = {}, roleOptions = []) {
  const roles = Array.isArray(roleOptions) ? roleOptions : [];
  const selectedRole = participant.stakeholder_role_id || participant.role || "";
  const responseCount = Number(participant.response_count || 0);
  return `
    <div style="position:fixed;inset:0;background:rgba(0,0,0,0.72);z-index:9999;display:flex;align-items:center;justify-content:center;padding:1.5rem;" data-participant-edit-backdrop>
      <form data-participant-edit-form style="width:min(560px,100%);max-height:90vh;overflow:auto;background:#111109;border:1px solid rgba(200,169,110,0.25);border-radius:12px;padding:1.25rem;color:#E8E4DC;box-shadow:0 24px 80px rgba(0,0,0,0.45);">
        <input type="hidden" name="participant_id" value="${escapeHtml(participant.participant_id || participant.id || "")}">
        <input type="hidden" name="original_role" value="${escapeHtml(selectedRole)}">
        <input type="hidden" name="response_count" value="${responseCount}">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;margin-bottom:1rem;">
          <div>
            <p style="margin:0 0 4px;color:rgba(200,169,110,0.55);font-size:11px;text-transform:uppercase;letter-spacing:0.08em;">Participant</p>
            <h3 style="margin:0;color:#E8E4DC;font-size:18px;">Edit Participant</h3>
          </div>
          <button type="button" data-participant-edit-close style="background:none;border:none;color:rgba(232,228,220,0.55);font-size:20px;cursor:pointer;">x</button>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
          <label style="display:flex;flex-direction:column;gap:6px;font-size:12px;color:rgba(200,169,110,0.6);grid-column:1 / -1;">Name
            <input name="name" type="text" value="${escapeHtml(participant.full_name || participant.name || "")}" required style="background:#1A1A18;border:1px solid rgba(200,169,110,0.28);border-radius:8px;color:#E8E4DC;padding:10px;">
          </label>
          <label style="display:flex;flex-direction:column;gap:6px;font-size:12px;color:rgba(200,169,110,0.6);grid-column:1 / -1;">Email
            <input name="email" type="email" value="${escapeHtml(participant.email || "")}" style="background:#1A1A18;border:1px solid rgba(200,169,110,0.28);border-radius:8px;color:#E8E4DC;padding:10px;">
          </label>
          <label style="display:flex;flex-direction:column;gap:6px;font-size:12px;color:rgba(200,169,110,0.6);">Role
            <select name="role" data-participant-edit-role style="background:#1A1A18;border:1px solid rgba(200,169,110,0.28);border-radius:8px;color:#E8E4DC;padding:10px;">
              ${roles.map((role) => {
                const value = role.id || role.value || "";
                const label = role.name || role.label || value;
                return `<option value="${escapeHtml(value)}" ${String(value) === String(selectedRole) ? "selected" : ""}>${escapeHtml(label)}</option>`;
              }).join("")}
            </select>
          </label>
          <label style="display:flex;flex-direction:column;gap:6px;font-size:12px;color:rgba(200,169,110,0.6);">Group
            <input name="group" type="text" value="${escapeHtml(participant.stakeholder_group_name || participant.stakeholder_group_id || "")}" style="background:#1A1A18;border:1px solid rgba(200,169,110,0.28);border-radius:8px;color:#E8E4DC;padding:10px;">
          </label>
        </div>
        <div data-participant-role-warning style="display:none;margin-top:12px;background:rgba(224,123,90,0.08);border:1px solid rgba(224,123,90,0.28);border-radius:8px;padding:12px;color:rgba(232,228,220,0.82);font-size:13px;line-height:1.5;">
          <strong style="display:block;color:#E07B5A;margin-bottom:4px;">Role change warning</strong>
          This participant has already answered questions. Changing their role will assign them a new question set. Their existing responses will be preserved but they will need to complete a new session.
          <div style="display:flex;gap:8px;margin-top:10px;">
            <button type="button" data-participant-confirm-role-change style="background:#E07B5A;color:#1A0F0A;border:none;border-radius:8px;padding:8px 10px;cursor:pointer;">Change Role</button>
            <button type="button" data-participant-keep-role style="background:transparent;color:#E8E4DC;border:1px solid rgba(232,228,220,0.25);border-radius:8px;padding:8px 10px;cursor:pointer;">Keep Current Role</button>
          </div>
        </div>
        <div data-participant-edit-error style="display:none;margin-top:12px;color:#E07B5A;font-size:13px;"></div>
        <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:1rem;">
          <button type="button" data-participant-edit-close style="background:transparent;color:rgba(232,228,220,0.7);border:1px solid rgba(232,228,220,0.18);border-radius:8px;padding:10px 14px;cursor:pointer;">Cancel</button>
          <button type="submit" style="background:#C8A96E;color:#1A1508;border:none;border-radius:8px;padding:10px 14px;font-weight:600;cursor:pointer;">Save Participant</button>
        </div>
      </form>
    </div>
  `;
...
export function renderParticipantsSectionComponent(
  ...
  return `
    <section class="engine-block participants-panel">
      <style>
        .participants-panel .participant-form {
          background: #111109 !important;
          border: 1px solid rgba(200,169,110,0.18) !important;
          color: #E8E4DC !important;
        }

        .participants-panel .participant-form input,
        .participants-panel .participant-form select,
        .participants-panel .participant-form textarea {
        ...
        .participants-panel .participant-form input::placeholder,
        .participants-panel .participant-form textarea::placeholder {
        ...
        <table class="compact-table">
          ...
          <tbody>
            ${visibleParticipants.map((participant) => {
              ...
              const participantCompleted = ["completed", "analyzed"].includes(String(participant.status || "").toLowerCase());
              const completedTooltip = "Participant has already completed this assessment";
              const completedDisabled = participantCompleted ? `disabled title="${completedTooltip}" aria-disabled="true"` : "";
              return `
              <tr>
                ...
                <td>
                  <div class="row-actions">
                    ...
                    ${onSendInvite ? `<button class="ghost-button" data-component-send-invite="${escapeHtml(participant.participant_id)}" type="button" ${completedDisabled}>${participant.invite_count ? "Resend Invite" : "Send Invite"}</button>` : ""}
                    ...
                    ${onSendReminder ? `<button class="ghost-button" data-component-send-reminder="${escapeHtml(participant.participant_id)}" type="button" ${completedDisabled}>Send Reminder</button>` : ""}
