...
function coverageComponentRow(label, component = {}) {
  ...
  const glossaryKey = {
    ...
    Depth: 'depth_score',
    'Response mix': 'response_mix_score',
    'Activity fit': 'activity_fit_score'
  }[label];
  const labelHtml = glossaryKey
    ? renderGlossaryTooltip(glossaryKey, label, description || `${label} coverage metric`)
    : escapeHtml(label);
  return `
    <div title="${escapeHtml(description)}" style="display:grid;grid-template-columns:150px 1fr 70px;gap:10px;align-items:center;margin:10px 0;padding:8px 0;border-bottom:0.5px solid rgba(200,169,110,0.08);">
      <span style="font-size:12px;color:var(--color-text-muted);">${labelHtml}</span>
  ...
  return `
    <section style="background:#111109;border:0.5px solid rgba(200,169,110,0.18);border-radius:12px;padding:1rem;margin-top:12px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
        <strong style="font-size:13px;color:#E8E4DC;">${renderGlossaryTooltip('quality_score', 'Quality Score', 'Question KPI linkage and finding potential quality.')}</strong>
...
function renderKpiManagement(kpis = [], activities = []) {
  ...
  const categoryForKpi = (kpi = {}) => {
    const activitiesText = array(kpi.applicable_activities).join(' ');
    const haystack = `${activitiesText} ${kpi.key || ''} ${kpi.name || ''} ${kpi.unit || ''}`.toLowerCase();
    if (/roadside|dispatch|mobile|sla/.test(haystack)) return 'RESPONSE & DISPATCH';
    if (/inventory|retail|wholesale/.test(haystack)) return 'INVENTORY';
    if (/fleet/.test(haystack)) return 'FLEET';
    if (/retreading|retread/.test(haystack)) return 'RETREADING';
    if (/technician|jobs/.test(haystack)) return 'WORKFORCE';
    if (/retail|ecommerce|e-commerce|dealer/.test(haystack)) return 'RETAIL & SALES';
    if (/import|supplier/.test(haystack)) return 'IMPORT & SUPPLY';
    if (/alignment|maintenance|monitoring/.test(haystack)) return 'SERVICES';
    if (/manufacturing|recycling|recycl/.test(haystack)) return 'MANUFACTURING';
    return 'OTHER';
  };
  ...
  const grouped = list.reduce((groups, kpi) => {
    const group = categoryForKpi(kpi);
  ...
  return `
    <section style="background:#111109;border:0.5px solid rgba(200,169,110,0.18);border-radius:12px;overflow:hidden;margin-bottom:12px;">
      ...
      ${categories.filter((category) => grouped[category]?.length).map((group) => `
        <div>
          <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.08em;color:rgba(200,169,110,0.4);padding:10px 1.25rem 4px;border-bottom:0.5px solid rgba(200,169,110,0.08);">${escapeHtml(group)}</div>
          ${grouped[group].map((kpi) => `
            <div style="display:flex;align-items:center;gap:12px;padding:8px 1.25rem;border-bottom:0.5px solid rgba(200,169,110,0.05);">
              <div style="flex:1;min-width:0;">
                <div style="font-size:13px;color:#E8E4DC;">${escapeHtml(kpi.name || kpi.key || '')}</div>
                ${kpi.name_tr ? `<div style="font-size:11px;color:rgba(200,169,110,0.5);margin-top:2px;">${escapeHtml(kpi.name_tr)}</div>` : ''}
              </div>
              <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;justify-content:flex-end;">
                <span style="font-size:11px;padding:2px 8px;border-radius:12px;background:rgba(200,169,110,0.06);border:0.5px solid rgba(200,169,110,0.15);color:${kpi.target_direction === 'lower_is_better' ? '#D9974A' : 'rgba(140,210,140,0.9)'};">${kpi.target_direction === 'lower_is_better' ? '↓' : '↑'} ${escapeHtml(kpi.target_direction || '')}</span>
                ${kpi.unit ? `<span style="font-size:11px;padding:2px 8px;border-radius:12px;background:rgba(200,169,110,0.06);border:0.5px solid rgba(200,169,110,0.15);color:rgba(200,169,110,0.65);">${escapeHtml(unitLabel(kpi.unit))}</span>` : ''}
              </div>
...
export function renderFrameworkPreview(
  ...
  readiness,
  activating,
  { onBack, onActivate, onOpenBank, auditState = {} } = {}
) {
  ...
  return `
    <section class="panel framework-builder framework-preview">
      ...
      ${renderFrameworkQualityAudit(bankList, auditState)}
...
function renderFrameworkQualityAudit(banks, auditState = {}) {
  const auditResults = array(auditState.results);
  const bankCount = array(banks).length;
  const totalQuestions = array(banks).reduce((sum, bank) => sum + Number(bank.question_count || 0), 0);
  const keepTotal = auditResults.reduce((sum, bank) => sum + array(bank.keep).length, 0);
  const removeTotal = auditResults.reduce((sum, bank) => sum + array(bank.remove).length, 0);
  const replaceTotal = auditResults.reduce((sum, bank) => sum + array(bank.replace).length, 0);
  const weakBanks = array(banks).filter((bank) => {
    const missing = array(bank.coverage?.breadth?.missing_domains);
    return missing.length || ['D', 'F'].includes(bank.coverage?.grade);
  });
  return `
    <section class="card framework-quality-audit">
      <div class="panel-header compact">
        <div>
          <h3>Framework quality audit</h3>
          <span>AI reviews all question banks for redundancy and coverage</span>
        </div>
      </div>
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:12px;">
        <button class="primary-button" type="button" data-framework-action="audit-all-banks" ${auditState.auditing ? 'disabled' : ''}>
          ${auditState.auditing ? `Auditing ${bankCount} banks...` : 'Audit All Banks'}
        </button>
        <button class="secondary-button" type="button" data-framework-action="complete-all-banks" ${auditState.completing ? 'disabled' : ''}>
          ${auditState.completing ? 'Generating questions for weak banks...' : 'Complete All Banks'}
        </button>
      </div>
      <p class="upload-note" style="margin-top:10px;">
        ${bankCount} banks · ${totalQuestions} questions · ${weakBanks.length} banks currently need coverage attention
      </p>
      ${auditResults.length ? `
        <div class="success-banner" style="margin-top:16px;">
          <h3>AUDIT COMPLETE</h3>
          <p>${auditResults.length} banks · ${auditResults.reduce((sum, bank) => sum + Number(bank.total || 0), 0)} questions reviewed · Recommended: Keep ${keepTotal} · Remove ${removeTotal} · Replace ${replaceTotal}</p>
          <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:12px;">
            <button class="primary-button" type="button" data-framework-action="apply-audit">Apply All Recommendations</button>
            <button class="secondary-button" type="button" data-framework-action="cancel-audit">Cancel - Keep Everything</button>
          </div>
        </div>
        <div style="margin-top:14px;display:grid;gap:12px;">
          ${auditResults.map((bank) => renderAuditBankResult(bank)).join('')}
        </div>
      ` : ''}
      ${array(auditState.completionResults).length ? `
        <div class="success-banner" style="margin-top:16px;">
          <h3>COMPLETE ALL BANKS RESULTS</h3>
          <p>${auditState.totalGenerated || 0} questions added across ${array(auditState.completionResults).length} banks.</p>
          ${array(auditState.completionResults).map((bank) => `
            <p>${escapeHtml(bank.role_name)}: Generated ${Number(bank.questions_generated || 0)} questions${array(bank.domains).length ? ` covering ${array(bank.domains).map(escapeHtml).join(', ')}` : ''}</p>
          `).join('')}
        </div>
      ` : ''}
    </section>
...
function renderAuditBankResult(bank) {
  const keep = array(bank.keep);
  const remove = array(bank.remove);
  const replace = array(bank.replace);
  return `
    <article class="upload-result">
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;">
        <div>
          <strong>${escapeHtml(bank.role_name || 'Role bank')}</strong>
          <p>Keep ${keep.length} · Remove ${remove.length} · Replace ${replace.length}</p>
        </div>
        <button class="secondary-button" type="button" data-framework-action="apply-bank-audit" data-bank-id="${escapeHtml(bank.bank_id)}">Apply for this bank</button>
      </div>
      ${bank.skipped ? `<p class="upload-note">${escapeHtml(bank.reason || 'Skipped')}</p>` : ''}
      ${remove.length ? `
        <details style="margin-top:10px;">
          <summary>Questions to remove (${remove.length})</summary>
          ${remove.map((item) => `
            <p><strong>${escapeHtml(item.question_id)}</strong> — ${escapeHtml(item.reason || '')}</p>
          `).join('')}
        </details>
      ` : ''}
      ${replace.length ? `
        <details style="margin-top:10px;">
          <summary>Questions to replace (${replace.length})</summary>
          ${replace.map((item) => `
