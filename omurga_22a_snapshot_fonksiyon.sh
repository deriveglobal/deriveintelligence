#!/usr/bin/env bash
# OMURGA 22a — metrik_snapshot_al(tenant): cron mantığı tek-kiracı fonksiyon. KUR + KRB'de TEST.
# Cron'un birebir aynısı, sadece WHERE tenant filtresi. DB-only, rebuild yok.
set -uo pipefail
PSQL="docker exec -i krb-assessment-postgres psql -U assessment_app -d assessment_platform"
T='f8a5d20f-ecf8-4ce2-a492-69268fbb03fa'
hr(){ printf '\n════════ %s ════════\n' "$1"; }

hr "1. FONKSİYON — metrik_snapshot_al(p_tenant) [cron A-E, tek kiracı]"
$PSQL -v ON_ERROR_STOP=1 <<'SQL' || { echo "  ✗ fonksiyon kurulamadı"; exit 1; }
CREATE OR REPLACE FUNCTION metrik_snapshot_al(p_tenant uuid) RETURNS void AS $fn$
BEGIN
...
$fn$ LANGUAGE plpgsql;
SQL
echo "  ✅ metrik_snapshot_al kuruldu"

hr "2. ÖNCE — current ay değerleri (fonksiyondan önce)"
$PSQL -c "SELECT metrik, round(deger) deger, guven, hesaplanma_at::timestamp(0) FROM bi_metrik_gecmis
          WHERE tenant_id='$T'::uuid AND boyut_tipi='sirket' AND donem=date_trunc('month',CURRENT_DATE)::date
          ORDER BY metrik;" 2>&1 | sed 's/^/  /'

hr "3. ÇALIŞTIR — metrik_snapshot_al(KRB)"
$PSQL -c "SELECT metrik_snapshot_al('$T'::uuid);" 2>&1 | sed 's/^/  /'

hr "4. SONRA — current ay (canlı ekranla tutmalı: stok~235M · dso~130 · alacak~209M · borç~403M · net~41M)"
$PSQL -c "SELECT metrik, round(deger) deger, guven, hesaplanma_at::timestamp(0) FROM bi_metrik_gecmis
          WHERE tenant_id='$T'::uuid AND boyut_tipi='sirket' AND donem=date_trunc('month',CURRENT_DATE)::date
          ORDER BY metrik;" 2>&1 | sed 's/^/  /'

hr "BITTI — değerler ekranla tutuyorsa fonksiyon doğru; sonra endpoint hook + cron DRY."
