...
// ═══════════════════════════════════════════════════════════════════════════
// SAHA — Saha ziyaretleri /* DEKRB_SAHA_V1 */ + iskonto onay modülü (mobile-first shell)
...
function layout() {
  const tabs = [
    ...
    ...(S.isYonetim ? [["rep-aktivite", "📡", "Aktivite"]] : []),
...
async function vZiyaretler() {
  try {
    ...
    /* ZIYARET_SIRA_V1 — kesin azalan (en yeni ustte) */
    const _zk = z => (Date.parse(z.checkin_at || z.ziyaret_tarihi || z.planlanan_tarih || "") || 0);
    ziyaretler.sort((a, b) => _zk(b) - _zk(a));
    ...
    return `
      <div style="margin-top:5px;padding:5px 7px;background:#fffbeb;border-radius:5px;border-left:3px solid #f59e0b">
        ...
        <div style="font-size:11px;color:#64748b;margin-top:2px">
          ...
          henüz yüklenmedi. Bu maliyet, <b>firmanın kendi alış faturasından türetildi</b>
          ...
          çalışıyor; <b>primi firma ayrıca fatura ediyor</b>, alış faturasına girmiyor.
...
async function vRapor() {
  ...
  function rpHarita() {
      ...
      <div style="padding:10px 12px 80px">
        ...
        <div style="background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:14px">
          ...
          <div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap" id="ss-kapsam-bar">
            ${[["tum","🏢 Tüm "+(me.tenantAlias||me.tenantName||'Saha')],["temsilci","👤 Temsilci"],["durum","⚠ Pasif/Riskli"],["musteri","🏬 Müşteri"]].map(([v,l]) =>
    ...
    document.getElementById("ss-analiz-btn")?.addEventListener("click", async () => {
      ...
      if (!btn || !sonuc) return;
      if (!ssKapsam) { uyari("Önce bir kapsam seçin (Tüm "+(me.tenantAlias||me.tenantName||'Saha')+", Temsilci, vb.)"); return; }
