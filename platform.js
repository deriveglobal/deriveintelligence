...
async function loadPlatformSettings() {
  ...
  markActiveNav();
  try {
    const res = await fetch('/api/platform/settings', { headers: authHeaders() });
...
function renderPlatformAssessmentWorkspace() {
  ...
  const workspaceHtml = renderAssessmentWorkspaceComponent(
    ...
    '',
    {
      ...
      onApproveRecommendation: (id) => callbacks().onApproveRecommendation?.(id),
      onRejectRecommendation: (id) => callbacks().onRejectRecommendation?.(id),
      onRecommendationStatus: (id, status) => callbacks().onRecommendationStatus?.(id, status),
...
function wirePlatformShell(container) {
  ...
  container.addEventListener?.('click', (event) => {
    ...
    if (event.target.closest?.('[data-component-merge-selected-findings]')) {
      ...
      if (platformState.selectedFindingId) callbacks().onMergeFindings?.([platformState.selectedFindingId, ...ids]);
      return;
    }
    ...
    if (recAction) {
      ...
      if (action === 'accept') callbacks().onRecommendationStatus?.(recommendationId, 'accepted');
      if (action === 'progress') callbacks().onRecommendationStatus?.(recommendationId, 'in_progress');
      if (action === 'complete') callbacks().onRecommendationStatus?.(recommendationId, 'completed');
      if (action === 'notes') callbacks().onRecommendationNotes?.(recommendationId, '');
      if (action === 'outcome') callbacks().onTrackOutcome?.(recommendationId, '');
      return;
    }
