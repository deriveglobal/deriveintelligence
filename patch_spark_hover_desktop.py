# -*- coding: utf-8 -*-
# RAPOR_SPARK_HOVER_DK_V1 (masaüstü) — Özet hero sparkline hover: dikey çizgi + tarih·ziyaret tooltip.
import sys
F = sys.argv[1] if len(sys.argv) > 1 else "shells/saha_desktop.js"
s = open(F, encoding="utf-8").read()
if "RAPOR_SPARK_HOVER_DK_V1" in s:
    print("[skip] zaten yamali"); sys.exit(0)

c_old = ".rpc-spark{margin-top:12px}"
c_new = ".rpc-spark{margin-top:12px;position:relative;cursor:crosshair}\n      .rpc-xline{position:absolute;top:2px;bottom:2px;width:1px;background:#2563eb;opacity:0;pointer-events:none;transition:opacity .1s}/*RAPOR_SPARK_HOVER_DK_V1*/"
assert s.count(c_old) == 1, "css anchor=%d" % s.count(c_old)
s = s.replace(c_old, c_new, 1)

h_old = '${(ozet.gunluk && ozet.gunluk.length > 1) ? `<div class="rpc-spark">${_rpSpark(ozet.gunluk.map(g => Number(g.ziyaret) || 0))}</div>` : ""}'
h_new = '${(ozet.gunluk && ozet.gunluk.length > 1) ? `<div class="rpc-spark" data-g="${esc(JSON.stringify(ozet.gunluk.map(g => ({ t: String(g.tarih || "").slice(0,10), v: Number(g.ziyaret) || 0 }))))}">${_rpSpark(ozet.gunluk.map(g => Number(g.ziyaret) || 0))}<div class="rpc-xline"></div></div>` : ""}'
assert s.count(h_old) == 1, "hero anchor=%d" % s.count(h_old)
s = s.replace(h_old, h_new, 1)

w_anchor = "    el2.innerHTML = `<!--RAPOR_UI_DK_V1-->"
WIRE = r'''    const _rpSparkWire = (root) => {  /* RAPOR_SPARK_HOVER_DK_V1 */
...
print("[done] RAPOR_SPARK_HOVER_DK_V1 (masaüstü)")
